...
protected:
  Endpoint(std::unique_ptr<Connection> &conn, std::shared_ptr<ILog> log)
      : m_conn(std::move(conn)), m_log(log) {}
  ...
  std::shared_ptr<Packet> RecvPacket(uint32_t commandId = 0) {
    ...
    if (commandId != 0) {
      if (commandId != packet->CommandId())
        throw std::runtime_error("Endpoint::RecvPacket - Expected packet of "
                                 "one type, but got something different.");
    }
    return packet;
  }

  template <class T> std::shared_ptr<T> RecvPacket() {
    std::shared_ptr<Packet> packet = Packet::Recv(m_conn.get());

    std::shared_ptr<T> res = std::dynamic_pointer_cast<T>(packet);
    if (!res)
      Throw<std::runtime_error>()
          << "Endpoint::RecvPacket - Expected packet of type "
          << typeid(T).name() << " but got " << typeid(*packet).name() << ".";
    return res;
  }
