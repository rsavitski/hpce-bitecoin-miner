...
class ConnectionOverSocket : public Connection {
private:
  friend std::unique_ptr<Connection>
  bitecoin::OpenConnection_Socket(std::vector<std::string> &spec);

  int m_socket;
  uint64_t m_sendOffset, m_recvOffset;

  ConnectionOverSocket(int _socket)
      : m_socket(_socket), m_sendOffset(0), m_recvOffset(0) {
    if (m_socket == -1)
...
public:
  ...
  virtual void Send(size_t cbData, const void *pData) override {
    ...
    while (todo) {
      ...
      if (done <= 0) {
        ...
        acc << "Send - Received error while reading writing to socket (" << e
            << " = " << strerror(e) << ")";
  ...
  virtual void Recv(size_t cbData, void *pData) override {
    ...
    while (todo) {
      ...
      if (done <= 0) {
        int e = errno;
        std::stringstream acc;
        acc << "Recv - Received error while reading reading from socket (" << e
            << " = " << strerror(e) << ")";
        throw std::runtime_error(acc.str());
      }
      m_recvOffset += done;
  ...
  virtual uint64_t SendOffset() const override { return m_sendOffset; }

  virtual uint64_t RecvOffset() const override { return m_recvOffset; }
};

}; // detail

std::unique_ptr<Connection>
OpenConnection_Socket(std::vector<std::string> &spec) {
  if (spec.size() < 1)
    throw std::invalid_argument(
        "OpenConnection_Socket - Not enough spec arguments.");

  if (spec[0] == "tcp-server") {
    if (spec.size() != 2)
      throw std::invalid_argument(
          "OpenConnection_Socket - Spec should be 'tcp-server portNum'.");
    ...
    if (sockListen == -1) {
      Throw<std::runtime_error>()
          << "OpenConnection_Socket - Couldn't create socket, errno=" << errno;
    }
    ...
    if (bind(sockListen, (struct sockaddr *)&server_addr, sizeof(server_addr)) <
      ...
      close(sockListen);
      Throw<std::runtime_error>()
          << "OpenConnection_Socket - Couldn't bind server socket, errno=" << e;
    }
    ...
    if (listen(sockListen, 1) < 0) {
      ...
      close(sockListen);
      Throw<std::runtime_error>()
          << "OpenConnection_Socket - Couldn't listen on server socket, errno="
          << e;
    }

    struct sockaddr_in client_addr;
    socklen_t client_addr_length = sizeof(client_addr);
    int fd = accept(sockListen, (struct sockaddr *)&client_addr,
                    &client_addr_length);
    if (fd < 0) {
      int e = errno;
      close(sockListen);
      Throw<std::runtime_error>()
          << "OpenConnection_Socket - Error on server accept, errno=" << e;
    }
    ...
    if (spec.size() != 3)
      throw std::invalid_argument(
          "OpenConnection_Socket - Spec should be 'tcp-client address port'.");
    ...
    if (e != 0) {
      Throw<std::runtime_error>() << "OpenConnection_Socket - Resolving addr='"
                                  << addr << "', port='" << port
                                  << "', got err=" << gai_strerror(e);
    }
    ...
    if (curr == NULL) {
      Throw<std::runtime_error>()
          << "OpenConnection_Socket - Couldn't connect, errno=" << e;
    }
    ...
    Throw<std::runtime_error>()
        << "OpenConnection_Socket - Didn't understand spec header '" << spec[0]
        << "'.";
    return 0;
  }
}
