#ifndef bitecoin_protocol_connection_hpp
#define bitecoin_protocol_connection_hpp
  ...
  void CheckString(unsigned n, const char *data) const {
    for (unsigned i = 0; i < n; i++) {
      if (!(isprint(data[i]) || isspace(data[i])))
        throw std::runtime_error("Connection::CheckString - Attempt to send or "
                                 "receive non printable ASCII character.");
    }
  }
...
public:
  ...
  void Send(const std::string &val) {
    ...
    if (length != val.size())
      throw std::runtime_error(
          "Connection::Send - String has more than 2^32 characters.");
  ...
  template <class T> void Send(const T &x) { x.Send(this); }

  template <class T> void Recv(T &x) { x.Recv(this); }

  void Send(const std::vector<uint8_t> &v) {
    uint32_t length = v.size();
    if (length != v.size())
      throw std::invalid_argument(
          "Connection::Send - Vector has more than 32 elements.");
    Send(length);
    Send(length, &v[0]);
  }
  ...
  template <class T> void Send(const std::vector<T> &v) {
    uint32_t length = v.size();
    if (length != v.size())
      throw std::invalid_argument(
          "Connection::Send - Vector has more than 32 elements.");
  ...
  template <class T> void Recv(std::vector<T> &v) {
    uint32_t length = 0;
    Recv(length);
    v.resize(length);
    for (unsigned i = 0; i < length; i++) {
...
std::unique_ptr<Connection> OpenConnection(std::vector<std::string> &spec) {
  ...
  if (spec[0] == "file") {
    ...
    throw std::invalid_argument(
        "OpenConnection - Didn't understand connection header '" + spec[0] +
        "'.");
  }
}
...
template <class TException> class throw_helper {
private:
  bool m_valid;
...
public:
  throw_helper() : m_valid(true) {}

  throw_helper(throw_helper &&src) : m_valid(src.m_valid) {
    src.m_valid = false;
  }

  template <class T> std::stringstream &operator<<(const T &x) {
    m_acc << x;
    return m_acc;
  }
...
template <class T> detail::throw_helper<T> Throw() {
  return detail::throw_helper<T>();
}

}; // bitecoin

#include "bitecoin_connection_file.hpp"
#include "bitecoin_connection_socket.hpp"
