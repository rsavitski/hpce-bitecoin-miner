...
bigint_t HashReference(const Packet_ServerBeginRound *pParams,
                       unsigned nIndices, const uint32_t *pIndices) {
  if (nIndices > pParams->maxIndices)
    throw std::invalid_argument(
        "HashReference - Too many indices for parameter set.");
  ...
  for (unsigned i = 0; i < nIndices; i++) {
    if (i > 0) {
      if (pIndices[i - 1] >= pIndices[i])
        throw std::invalid_argument("HashReference - Indices are not in "
...
/*! This is used to choose the winner. It is somewhat biased against the very
   fastest
        people, so that it is still possible for slow people to occasionally win
   a coin.
        \param rng returns an double-precision uniform random in [0,1)
        \note It is basically choosing in proportion to the sqrt of the inverse
   fitness.
                So if there are three people, with effective hash rates of 1000,
   100, and 10,
...
unsigned ChooseWinner(const std::vector<bigint_t> &choices, TRng &rng) {
  ...
  for (unsigned i = 0; i < choices.size(); i++) {
    values.push_back(
        std::make_pair(1.0 / wide_as_double(8, choices[i].limbs), i));
  }
