...
private:
  ...
  void CheckSubmission(const Packet_ServerBeginRound *pBeginRound,
                       const submission_t &subClient) {
    Log(Log_Debug, "Starting to re-hash data.\n");
    bigint_t correct = HashReference(pBeginRound, subClient.solution.size(),
                                     &subClient.solution[0]);
    Log(Log_Debug, "Rehash done.\n");

    if (memcmp(correct.limbs, subClient.proof, BIGINT_LENGTH)) {
      Log(Log_Error,
          "CheckSubmission for submission from %s, proof is not correct.",
          subClient.clientId.c_str());
...
public:
  EndpointServer(std::string exchangeId, std::string serverId,
                 std::unique_ptr<Connection> &conn, int logLevel = 1)
      : Endpoint(conn, std::make_shared<LogDest>(exchangeId, logLevel)),
        m_exchangeId(exchangeId), m_serverId(serverId) {}
  ...
  void Run() {
    try {
      ...
      auto completeConnect = std::make_shared<Packet_ServerCompleteConnect>(
          m_exchangeId, m_serverId);
      ...
      while (1) {
        ...
        beginRound->roundId = 0xab2212f3;
        //beginRound->roundId = roundId;
        //beginRound->roundSalt = rand();
        beginRound->roundSalt = 0xbfafa232;
