#ifndef bitecoin_connection_file_hpp
#define bitecoin_connection_file_hpp
...
// Note: I could have just included <io.h> and msvc would whinge mightily, but
// carry on
...
class ConnectionOverFile : public Connection {
private:
  friend std::unique_ptr<Connection>
  bitecoin::OpenConnection_File(std::vector<std::string> &spec);

  int m_fdSend, m_fdRecv;
  uint64_t m_sendOffset, m_recvOffset;

  ConnectionOverFile(int fdSend, int fdRecv)
      : m_fdSend(fdSend), m_fdRecv(fdRecv), m_sendOffset(0), m_recvOffset(0) {
    if (fdSend == -1 || fdRecv == -1)
      throw std::invalid_argument(
          "ConnectionOverFile - one of the file descriptors is invalid.");
  }
  ...
  virtual void Send(size_t cbData, const void *pData) override {
    ...
    while (todo) {
      ...
      if (done <= 0) {
        ...
        acc << "Send - Received error while reading writing to file (" << e
            << " = " << strerror(e) << ")";
  ...
  virtual void Recv(size_t cbData, void *pData) override {
    ...
    while (todo) {
      ...
      if (done <= 0) {
        int e = errno;
        std::stringstream acc;
        acc << "Recv - Received error while reading reading from file (" << e
            << " = " << strerror(e) << ")";
        throw std::runtime_error(acc.str());
      }
      m_recvOffset += done;
...
OpenConnection_File(std::vector<std::string> &spec) {
  if (spec.size() != 3)
    throw std::runtime_error(
        "OpenConnection_File - Wrong number of components in spec.");

  if (spec[0] != "file")
    throw std::runtime_error(
        "OpenConnection_File - First component in spec is not 'file'.");
  ...
  if (fdSend == -1) {
    throw std::runtime_error("OpenConnection_File - Couldn't open '" + spec[2] +
                             "' for writing.");
  }
  ...
  if (fdRecv == -1) {
    close(fdSend);
    fdSend = -1;
    throw std::runtime_error("OpenConnection_File - Couldn't open '" + spec[1] +
                             "' for reading.");
  }

  // Roll on C++14
  return std::unique_ptr<Connection>(
      new detail::ConnectionOverFile(fdSend, fdRecv));
}
