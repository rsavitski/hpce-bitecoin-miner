...
private:
  ...
  send_context_t BeginSend(Connection *pConnection) const {
    send_context_t context = { Length(), uint32_t(clock() + rand()),
    ...
    if (context.length < 20)
      throw std::logic_error(
          "Packet::BeginSend - Cannot have a length of less than 20 bytes.");
  ...
  void EndSend(Connection *pConnection, const send_context_t &ctxt) const {
    ...
    if (endOffset < ctxt.beginOffset)
      throw std::runtime_error("Packet::EndSend - 64-bit offset has wrapped, "
                               "cannot handle more than 2^64 bytes of data.");

    uint64_t sent = endOffset - ctxt.beginOffset;
    if (sent != ctxt.length) {
      std::stringstream acc;
      acc << "Packet::EndSend - Sent data count (" << sent
          << ") does not match what we said in header(" << ctxt.length << ")";
...
public:
  ...
  uint64_t Length() const { return PayloadLength() + 20; }
  ...
  static std::shared_ptr<Packet> Recv(Connection *pConnection) {
    ...
    // command, sentinelHeader);

    if (length < 20)
      throw std::runtime_error("Packet::Recv - Received packet length of < 20 "
                               "bytes, which is not possible.");
    ...
    if (sentinelHeader != sentinelFooter)
      throw std::runtime_error(
          "Packet::Recv - trailing sentinel does not match leading sentinel.");

    uint64_t endOffset = pConnection->RecvOffset();

    if (endOffset < beginOffset)
      throw std::runtime_error("Packet::Recv - Offset has wrapped, we don't "
                               "support more than 2^64 bytes sent.");
    if (endOffset - beginOffset != length)
      throw std::runtime_error("Packet::Recv - Sent bytes does not match what "
                               "we said in the header.");
...
enum {
  ...
  Command_ServerRequestBid =
      5, // Sent to all clients to tell them the server wants  a bid
  Command_ClientSendBid =
      6, // Sent by a client to indicate their response from a mining round
  Command_ServerCompleteRound = 7 // Sent to all clients once round has finished
};

/*! After this packet is sent the connection is effectively shut, no further
 * traffic is possible */
class Packet_ServerError : public Packet {
protected:
...
public:
  virtual uint32_t CommandId() const override { return Command_ServerError; }

  std::string errorMessage; // ASCII string identifying the error
};

class Packet_ClientBeginConnect : public Packet {
protected:
...
public:
  ...
  Packet_ClientBeginConnect(std::string _clientId, std::string _minerId)
      : protocolVersion(0), clientId(_clientId), minerId(_minerId) {}

  virtual uint32_t CommandId() const override {
    return Command_ClientBeginConnect;
  }

  uint32_t protocolVersion; // Indicates the level of protocol supported by the
                            // client
...
class Packet_ServerCompleteConnect : public Packet {
protected:
  virtual void RecvPayload(Connection *pConnection) {
...
public:
  Packet_ServerCompleteConnect(std::string _exchangeId = "<invalid>",
                               std::string _serverId = "<invalid>")
      : protocolVersion(0), serverId(_serverId), exchangeId(_exchangeId) {}

  virtual uint32_t CommandId() const override {
    return Command_ServerCompleteConnect;
  }

  uint32_t protocolVersion; // Indicates the protocol level being used (wil be
                            // <= level indicated by client). Network order.
  std::string serverId;     // ASCII string identifying server software
  std::string exchangeId;   // ASCII string identifying the exchange
};

class Packet_ServerBeginRound : public Packet {
protected:
  virtual void RecvPayload(Connection *pConnection) override {
...
public:
  ...
  std::vector<uint8_t> chainData; // Chain data. On the wire consists of 64-bit
                                  // length, followed by bytes of chain data
...
class Packet_ServerRequestBid : public Packet {
protected:
  virtual void RecvPayload(Connection *pConnection) override {
...
public:
  ...
  timestamp_t timeStampRequestBids; // When the server considers the round to
                                    // have started
  timestamp_t timeStampReceiveBids; // When the server expects all responses by
};

class Packet_ClientSendBid : public Packet {
protected:
  virtual void RecvPayload(Connection *pConnection) override {
    pConnection->Recv(roundId);
...
public:
  virtual uint32_t CommandId() const override { return Command_ClientSendBid; }
...
struct submission_t {
  ...
  uint64_t PayloadLength() const {
    return 4 + clientId.size() + 4 + solution.size() * 4 + BIGINT_LENGTH +
           sizeof(timestamp_t) * 2;
  }
};

class Packet_ServerCompleteRound : public Packet {
protected:
...
public:
  ...
  uint64_t roundId;
  submission_t winner;
  std::vector<submission_t> submissions; // On the wire is a 64-bit length,
...
std::shared_ptr<Packet> Packet::CreatePacket(uint32_t command) {
  ...
  default: {
    std::stringstream acc;
    acc << "Packet::CreatePacket - Received a packet command id (" << command
